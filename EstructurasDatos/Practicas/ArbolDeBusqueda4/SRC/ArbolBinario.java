...
public abstract class ArbolBinario<T> implements Coleccionable<T> {
    ...
    protected class Nodo<T> {
        ...
        public int altura() {
           ...
           //return 1 + padre.altura();
           return  1+ Math.max((hayIzquierdo())?(izquierdo).altura():0,(hayDerecho())?(derecho).altura():0) ;
        }
    ...
    public int altura() {
       if (esVacio()) {
         ...
         return (1 + Math.max((raiz.hayIzquierdo())?(raiz.izquierdo).altura():0,(raiz.hayDerecho())?(raiz.derecho).altura():0) );
       }
    }
